...
void main() {
  testWidgets('SignedInShell shows 4 destinations for non-creator', (tester) async {
    final api = FakeApi(
    ...
    await tester.pumpWidget(MaterialApp(home: SignedInShell(
      api: api,
  ...
  testWidgets('SignedInShell shows 4 destinations for creator with Create tab', (tester) async {
    final api = FakeApi(
      Future.value(const SystemDistributionInfo('1.0.0', 0)),
    );
    await api.signInAs(const MySummary(
  ...
  testWidgets('SignedInShell invokes onNavigate when a destination is tapped', (tester) async {
    final api = FakeApi(
      Future.value(const SystemDistributionInfo('1.0.0', 0)),
    );
    await api.signInAs(const MySummary(
