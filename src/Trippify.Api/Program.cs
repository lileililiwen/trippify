...
builder.Services.AddSingleton<IObjectStorage>(x => x.GetRequiredService<LocalProviders>()); builder.Services.AddSingleton<IEmailSender>(x => x.GetRequiredService<LocalProviders>()); builder.Services.AddSingleton<IMapProvider>(x => x.GetRequiredService<LocalProviders>()); builder.Services.AddSingleton<IPaymentGateway>(x => x.GetRequiredService<LocalProviders>()); builder.Services.AddSingleton<IAiAssistant>(x => x.GetRequiredService<LocalProviders>()); builder.Services.AddSingleton<IBackgroundJobQueue>(x => x.GetRequiredService<LocalProviders>()); builder.Services.AddSingleton<IClock, SystemClock>();
var app = builder.Build(); app.UseExceptionHandler(); app.Use(async (context, next) => { context.Response.Headers.Append("X-Correlation-Id", Activity.Current?.TraceId.ToString() ?? context.TraceIdentifier); await next(); }); app.UseRateLimiter(); app.UseAuthentication(); app.UseAuthorization(); app.UseSwagger(); app.UseSwaggerUI();
